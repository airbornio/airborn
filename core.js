...
window.prepareFile = function(file, options, callback, progress, createObjectURL) {
	...
	if(isHTML(extension) && options.bootstrap !== false) {
		...
		var data = [
			...
			"	var filenames = {};",
			"	var apikey = " + JSON.stringify(options.apikey || getAPIKey()) + ";",
			...
			"				'document.filenames = ' + JSON.stringify(filenames) + ';' +",
			"				'document.apikey = ' + JSON.stringify(apikey) + ';' +",
			"				'<\\/script>' + message.data.result[0]",
			"			);",
			"			document.close();",
(navigator.userAgent.indexOf('Firefox') !== -1 ?
			"			history.replaceState({}, '', ''); // Make refresh iframe work in Firefox"
: ""),
			"			if(window.parent !== window.top) window.parent.postMessage({action: 'wm.hideProgress', args: []}, '*');",
			"		}",
			"	});",
			"	window.top.postMessage({action: 'fs.prepareFile', args: " + JSON.stringify([file, _options]) + ", apikey: apikey}, '*');",
