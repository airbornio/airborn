...
window.prepareFile = function(file, options, callback, progress, createObjectURL) {
	...
	if(isHTML(extension) && options.bootstrap !== false) {
		...
		var data = [
			...
			'var messageID = Math.round(Math.random() * Date.now()).toString(16);',
			...
			'		return;',
			'	}',
			'	if(message.data.inReplyTo !== messageID) return;',
			...
			'	}',
			'});',
			'window.top.postMessage({action: "fs.prepareFile", args: ' + JSON.stringify([file, _options]) + ', apikey: document.apikey, messageID: messageID}, "*");',
